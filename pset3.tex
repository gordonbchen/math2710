...
\begin{solution}\\
Yes, this is a statement because the statement that 423 is prime is either true or false. The statement is false because 423 is divisible by 3, $423 / 3 = 141$.
\end{solution}
...
\begin{parts}
    ...
    \begin{solution}\\
    This is neither a tautology nor a contradiction. The statement is only true if P and Q are both true or both false. The statement can be true or false depending on the values of P and Q, so it is neither a tautology nor a contradiction.
    \end{solution}
    ...
    $\left[ P \Rightarrow (Q \wedge R) \right] \Rightarrow (P \Rightarrow Q )$

    $\equiv ((\sim P) \vee (Q \wedge R)) \Rightarrow (P \Rightarrow Q )$

    $\equiv (\sim ((\sim P) \vee (Q \wedge R))) \vee (P \Rightarrow Q )$

    $\equiv (\sim (((\sim P) \vee Q) \wedge ((\sim P) \vee R))) \vee (P \Rightarrow Q )$

    $\equiv (\sim ((P \Rightarrow Q) \wedge (P \Rightarrow R))) \vee (P \Rightarrow Q )$

    $\equiv (\sim (P \Rightarrow Q)) \vee (\sim (P \Rightarrow R)) \vee (P \Rightarrow Q )$

    $\equiv (\sim (P \Rightarrow Q)) \vee (P \Rightarrow Q ) \vee (\sim (P \Rightarrow R))$

    $\equiv T \vee (\sim (P \Rightarrow R))$
...
$\equiv (\sim P) \vee (Q \vee R) \qquad$ by definition of $\Rightarrow$.

$\equiv ((\sim P) \vee Q) \vee R \qquad$ by Associative Laws.

$\equiv (\sim (P \wedge (\sim Q))) \vee R \qquad$  by DeMorgan's Law.

$\equiv (P \wedge (\sim Q)) \Rightarrow R \qquad$ by definition of $\Rightarrow$.
\end{solution}
...
\begin{parts}
    ...
    \hline
    $P $& $Q$ & $\sim P$ & $\sim Q$ & $P \vee Q$ & $(\sim P) \vee (\sim Q)$ & $(P \vee Q) \wedge ((\sim P) \vee (\sim Q))$ & $P \veebar Q$ \\
    \hline
    T&T&F&F&T&F&F&F\\
    \hline
    T&F&F&T&T&T&T&T\\
    ...
    $\equiv ((P \wedge (\sim Q)) \vee (\sim P)) \wedge ((P \wedge (\sim Q)) \vee Q) \qquad$  by Distributive laws.

    $\equiv ((P \vee (\sim P)) \wedge ((\sim Q) \vee (\sim P))) \wedge ((P \vee Q) \wedge ((\sim Q) \vee Q)) \qquad$  by Distributive laws.

    $\equiv ((\sim Q) \vee (\sim P)) \wedge (P \vee Q) \qquad$  since $P \vee (\sim P)$ is a tautology.

    $\equiv (P \vee Q) \wedge ((\sim P) \vee (\sim Q)) \qquad$  rearranging.
    \end{solution}
